...
def carregar_dados():
    ...
    # Todas as colunas numéricas (incluindo odds e defesas)
    colunas_numericas = [
...
with tab1:
    ...
    if df_subset.empty:
        ...
        with colA:
            ...
            # Encontra os máximos absolutos para a normalização perfeita
            ...
            # Evita divisão por zero se algum valor máximo for zero ou NaN
            tetos = [
            ...
            # Normalização (Escala de 0 a 1) para esticar o gráfico no limite radial
            norm_man = [m / t if t > 0 else 0 for m, t in zip(med_man, tetos)]
            norm_vis = [v / t if t > 0 else 0 for v, t in zip(med_vis, tetos)]

            # Textos para exibir quando o rato passa por cima (valor original)
